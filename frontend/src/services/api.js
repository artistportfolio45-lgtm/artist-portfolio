...
export const artworkAPI = {
  ...
  scanDuplicates: (offset = 0, batchSize = 25) => api.post("/artworks/duplicates/scan", { offset, batchSize }, { timeout: 0 }),
  removeDuplicates: (ids) => api.post("/artworks/duplicates/remove", { confirm: true, ...(Array.isArray(ids) ? { ids } : {}) }, { timeout: 0 }),
  ...
  deleteBatchHistory: (uploadBatchId) => api.delete(`/artworks/upload-history/batches/${encodeURIComponent(uploadBatchId)}`),
  startDeletionJob: (ids, options = {}) => api.post("/artworks/deletion-jobs", { ids, deleteBatchHistory: options.deleteBatchHistory === true }),
